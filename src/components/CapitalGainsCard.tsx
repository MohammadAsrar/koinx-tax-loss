...
const CapitalGainsCard: React.FC<Props> = ({
  ...
  return (
    <div className={`capital-gains-card${highlight ? " highlight" : ""}`}>
      {" "}
      {/* Add styling later */}
      <h3>{title}</h3>
      ...
      <div className="realised-gains">
        {highlight ? (
          <>
            <div>
              <b>Effective Capital Gains:</b>{" "}
              <span>$ {effectiveGains.toLocaleString()}</span>
            </div>
          ...
          <div>
            <b>Realised Capital Gains:</b> $ {realisedGains.toLocaleString()}
          </div>
        )}
      </div>
    </div>
