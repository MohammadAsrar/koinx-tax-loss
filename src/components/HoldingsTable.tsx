...
const HoldingsTable: React.FC<Props> = ({
  ...
  const allSelected =
    holdings.length > 0 && holdings.every((h) => selected.has(h.coin));
  ...
  return (
    <div className="holdings-table-wrapper shadow-[0_2px_6.5px_rgba(0,0,0,0.6)] bg-gray-100">
      <h1 className="m-3 text-lg font-bold">Holdings</h1>
          ...
          {sortedHoldings.map((h) => (
            ...
            >
              ...
              <td>
                <div className="holding-info">
                  <div className="holding-amount">
                    {h.totalHolding} {h.coin}
                  </div>
                  <div className="holding-price">
                    $ {formatValue(h.averageBuyPrice)}/{h.coin}
                  </div>
                </div>
              </td>
              ...
              <td>
                <div
                  className={
                    h.stcg.gain >= 0 ? "gain-positive" : "gain-negative"
                  }
                >
                  {h.stcg.gain >= 0 ? "+" : "-"}$
                  {Math.abs(h.stcg.gain).toLocaleString()}
                </div>
              ...
              <td>
                <div
                  className={
                    h.ltcg.gain >= 0 ? "gain-positive" : "gain-negative"
                  }
                >
                  {h.ltcg.gain >= 0 ? "+" : "-"}$
                  {Math.abs(h.ltcg.gain).toLocaleString()}
                </div>
              ...
              <td>
                <div
                  className={
                    selected.has(h.coin) ? "sell-amount active" : "sell-amount"
                  }
                >
      ...
      {viewAll && onViewAll && (
        <div className="view-all-link hover:scale-105" onClick={onViewAll}>
          View all
        </div>
      )}
    </div>
