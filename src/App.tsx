...
function sumRealised(
  stcg: { profits: number; losses: number },
  ltcg: { profits: number; losses: number }
) {
  return (
    calcNetGains(stcg.profits, stcg.losses) +
    calcNetGains(ltcg.profits, ltcg.losses)
  );
}
...
const ThemeToggle: React.FC = () => {
  ...
  return (
    <button
      className="p-2 w-16 flex flex-col ml-auto rounded-full dark:bg-gray-500 dark:hover:bg-gray-600 transition-all duration-500 cursor-pointer"
      onClick={toggleTheme}
    >
      {theme === "dark" ? (
        <SunIcon className="text-yellow-400 self-end transition ease-in-out duration-500" />
      ) : (
        <MoonIcon className="text-gray-600 dark:text-white transition ease-in-out duration-500" />
      )}
    </button>
  );
};
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <p>
            Tax loss harvesting is a strategy that involves selling investments
            at a loss to offset capital gains tax liability.
          </p>
          ...
          <ol>
            ...
            <li>
              Use these losses to offset capital gains and reduce tax liability
            </li>
            <li>
              Optionally, reinvest in similar (but not "substantially
              identical") assets
            </li>
          </ol>
          <p>
            This tool helps you identify potential tax loss harvesting
            opportunities in your crypto portfolio and estimate the tax savings.
          </p>
        </div>
      </div>
    </div>
...
const AppContent: React.FC = () => {
  ...
  const {
    ...
    const postRealised =
      calcNetGains(stProfits, stLosses) + calcNetGains(ltProfits, ltLosses);
  ...
  if (!capitalGains)
    return <ErrorState message="Unable to load capital gains data" />;

  return (
    <div className="app-container">
      <header className="app-header">
        <img src="/Group 27342.png" alt="KoinX" className="logo" />
